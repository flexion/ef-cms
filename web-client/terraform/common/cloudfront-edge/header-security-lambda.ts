...
export const handler = (awsEvent, handlerContext, callback) => {
  ...
  const contentSecurityPolicy = [
    ...
    `connect-src blob: ${sentryUrl} ${subdomainsUrl} ${applicationUrl} ${s3Url} ${dynamsoftUrlProd} ${dynamsoftUrlTest} ${dynamsoftUrlStaging} ${localUrl} ${websocketUrl} ${localWebsocketUrl} ${pdfjsExpressUrl}`,
    "default-src 'none'",
    "manifest-src 'self'",
