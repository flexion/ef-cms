...
export const Header = ({
  ...
  headerHelper: {
    ustcSealLink: string;
    isLoggedIn: boolean;
    showMobileAccountMenu: boolean;
    showSearchInHeader: boolean;
    showAccountMenu: boolean;
    showVerifyEmailWarningNotification: boolean;
  };
  menuHelper: {
    isDocumentQCMenuOpen: boolean;
    isMessagesMenuOpen: boolean;
    isReportsMenuOpen: boolean;
    isAccountMenuOpen: boolean;
  };
  onResetHeaderAccordions: () => void;
  onSignOutUserInitiated: () => void;
  onToggleBetaBar: () => void;
  onToggleMobileMenu: () => void;
  showMobileMenu: boolean;
  templateHelper: { showBetaBar: boolean };
}) => {
  ...
  const keydown = event => {
    if (event.keyCode === 27) {
      return onResetHeaderAccordions();
    }
  };
  ...
  return (
    <>
      <div ref={headerRef}>
        {templateHelper.showBetaBar && BetaBar(onToggleBetaBar)}
