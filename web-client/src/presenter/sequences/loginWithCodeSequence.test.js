...
describe('loginWithCodeSequence', () => {
  ...
  it('should use the code to fetch the token and put it onto the store', async () => {
    await integrationTest.runSequence('loginWithCodeSequence', {
      code: 'abc',
    });
    expect(
    ...
    expect(integrationTest.getState('token')).toEqual(NEW_TOKEN);
    expect(integrationTest.getState('refreshToken')).toEqual(NEW_TOKEN);
    expect(integrationTest.getState('user')).toEqual(USER);
    expect(integrationTest.getState('refreshTokenInterval')).toBeDefined();
  });
});
