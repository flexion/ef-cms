...
const DOCKET_ENTRY_VALIDATION_RULE_KEYS = {
  ...
  filingDate: joi.when('eventCode', {
    is: JoiValidationConstants.STRING.valid(
      ...COURT_ISSUED_EVENT_CODES.map(({ eventCode }) => eventCode),
    ),
    otherwise: JoiValidationConstants.ISO_DATE.max('now')
      .required()
      .description('Date that this Document was filed.'),
    then: joi.when('servedAt', {
      is: joi.exist().not(null),
      otherwise: JoiValidationConstants.ISO_DATE.max('now')
        .required()
        .description('Date that this Document added to the docket record.'),
      then: joi.any().valid(joi.ref('servedAt')),
    }),
  }),
