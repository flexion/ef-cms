 ...
 * @param {string} providers.docketNumber the docket number of the case to update the contact
...
exports.updateContactInteractor = async (
  ...
  // TODO: where is the validation on this, and the updatePrimaryContactInteractor?
  ...
  const oldContact = cloneDeep(caseEntity.getPetitionerById(contactId));
  ...
  try {
    caseEntity.updatePetitioner(updatedContact);
  } catch (e) {
  ...
  if (!oldContact.isAddressSealed && changeOfAddressDocumentTypeToGenerate) {
    ...
    const changeOfAddressDocketEntry = new DocketEntry(
      {
        ...
        additionalInfo: `for ${oldContact.name}`,
    ...
    let privatePractitionersRepresentingContact = caseEntity.getPractitionersRepresenting(
      contactId,
    );
