...
import { navigateToCase } from '../support/pages/petition-qc';
...
describe('Batch Download Documents', () => {
  ...
  let zipName: string;
  let documentsCreatedWithCase;
  let caseNumberCreated: string;

  it('should create case for validating downloaded documents', () => {
    createAndServePaperPetition().then(
      ({ docketNumber, documentsCreated, name }) => {
        documentsCreatedWithCase = documentsCreated;
        zipName = `${docketNumber}, ${name}.zip`;
        caseNumberCreated = docketNumber;
      },
    );
  });
  ...
  it('should download all eligible documents that are not stricken with the printable docket record, with all files in a flat directory', () => {
    const downloadPath = Cypress.config('downloadsFolder');
    let expectedFileCount: number = documentsCreatedWithCase.length;
    const expectedSealedCount = 0;
    const expectedStrickenCount = 0;

    navigateToCase('petitionsclerk', caseNumberCreated);
    searchByDocketNumberInHeader(caseNumberCreated);
    confirmCountOfDocumentsToDownload(documentsCreatedWithCase.length);
  ...
  it('should download all eligible documents with some stricken and some sealed, labeling files and adding sealed docs to sealed directory', () => {
    const downloadPath = Cypress.config('downloadsFolder');
    const expectedFileCount: number = documentsCreatedWithCase.length;
