...
import {
  ...
  elasticsearchPerformanceIndexes,
  esIndexType,
} from './elasticsearch-indexes';
...
export const setupPerformanceIndexes = async ({
  client,
  environmentName,
}: {
  client: Client;
  ...
  await Promise.all(
    elasticsearchPerformanceIndexes.map(async (index: string) => {
      try {
        const { body: indexExists } = await client.indices.exists({
          index,
        });
        ...
        if (!indexExists) {
          return client.indices.create({
            body: {
              mappings: {
                dynamic: false,
                ...elasticsearchPerformanceMappings[index],
              },
              settings: esSettings,
            },
            index,
